...
"MSWin32-x64-multi-thread-10.0.20348.1366" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.20348.1366) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEALREADY WSAEDISCON WSAEWOULDBLOCK WSAEINVAL WSAEAFNOSUPPORT
	WSAENETUNREACH WSAEHOSTDOWN WSAEPROCLIM WSAELOOP WSAENETRESET
	WSAEPROVIDERFAILEDINIT WSAEINVALIDPROCTABLE WSAEREMOTE
	WSAEPROTONOSUPPORT WSAECANCELLED WSAETOOMANYREFS WSAEINTR WSAEDQUOT
	WSAEMSGSIZE WSAEBADF WSAEDESTADDRREQ WSAEINPROGRESS WSAEISCONN
	WSAEFAULT WSAEREFUSED WSAESOCKTNOSUPPORT WSAEHOSTUNREACH
	WSAEADDRNOTAVAIL WSAEADDRINUSE WSAENOPROTOOPT WSAEMFILE WSAESHUTDOWN
	WSAETIMEDOUT WSAENAMETOOLONG WSAEINVALIDPROVIDER WSAEUSERS
	WSAEOPNOTSUPP WSAEPROTOTYPE WSAENOTEMPTY WSAESTALE WSAEPFNOSUPPORT
	WSAENOTCONN WSAECONNREFUSED WSAENOBUFS WSAECONNRESET WSAENETDOWN
	WSAENOMORE WSAECONNABORTED WSAENOTSOCK WSAEACCES
    )],
);
